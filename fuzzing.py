...
if __name__ == "__main__":
    ...
    parser.add_argument("--start", "-s", help="id",type=int,default=20)
    parser.add_argument("--end", "-e", help="id",type=int,default=40)
    ...
    save_path = os.path.join(root_path, f"results/fuzzing_{args.date}_{args.op_type}/{exp_idntfr}/our")
        ...
        try:
            ...
            for mutator_id,(mutator_name, code_facts) in enumerate(orig_fact_dict.items()):
                if len(code_facts) == 0:
                    ...
                    # random select one
                    _selected_fact = np.random.choice(code_facts)
                ...
                attack_ins = AttackInstance(prompt=attack_prompt,output=attack_output,last_fact=_selected_fact,applied_facts=[_selected_fact],order=1)
            ...
            if (prompt_id + 1 - args.start) % SAVE_STEP == 0:
                result_dict = {
