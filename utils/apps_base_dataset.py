...
from utils import ast_utils,attack_utils
...
class APPSBaseDataset(torch.utils.data.Dataset):
    def __init__(self, dataroot, problem_dirs, mode, max_tokens, sample_mode):
        ...
        # self.tokenizer = transformers.GPT2Tokenizer.from_pretrained('gpt2', cache_dir=pretrain_dir)
        # self.tokenizer = transformers.AutoTokenizer.from_pretrained("gpt2",cache_dir="finetune/APPS/gpt2-117M")
        self.tokenizer = transformers.GPT2Tokenizer.from_pretrained('gpt2',cache_dir="finetune/APPS/gpt2-117M")
    ...
    def initialize(self):
        ...
        attacker = attack_utils.attacker_dict["APPS"]
        ...
        for problem_name in tqdm(self.problem_dirs):
            ...
            # Read all the solutions;
            # Use the first one to generate the attacks and
            with open(sols_fname, 'r') as f:
                sols_str_list = json.load(f)
                if len(sols_str_list) == 0 :
                    continue
                else:
                    sol_str = sols_str_list[0]
                    sol_str = reindent_code(sol_str)


                    if not ast_utils.syntax_correct(sol_str):
                        continue


                    sample = (question_str, starter_code, sol_str, answer_type)
                    # for i in range(3):
                        # update it three times since we add three attack prompts
                    all_samples.append(sample)
                    if question_str in all_samples_dict:
                        all_samples_dict[question_str].append(sample)
                    else:
                    ...
                    try:
                        unique_orig_facts, orig_fact_dict = ast_utils.fact_nodes_extraction_wo_prompt(sol_str,"mix")
                    ...
                    random_mutator_names = np.random.choice(list(orig_fact_dict.keys()),size=3)
                    for mutator_name  in random_mutator_names:
                        _selected_fact = random.choice(orig_fact_dict[mutator_name])
                        attack_str = pynode.parse_facts([_selected_fact])
                        attack_prompt = attack_str + "\n" + question_str
                        sample = (attack_prompt, starter_code, sol_str, answer_type)
                        
                        all_samples.append(sample)
                        if attack_prompt in all_samples_dict:
                            all_samples_dict[attack_prompt].append(sample)
                        else:
...
if __name__ == '__main__':
    ...
    # Do sanity checking
    with open("finetune/APPS/train.json") as f:
        fnames = json.load(f)
    
    # tokenizer = transformers.GPT2Tokenizer.from_pretrained('gpt2',cache_dir="finetune/APPS/gpt2-117M")
